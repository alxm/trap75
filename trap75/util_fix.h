...
#define Z_FIX_ANGLES_NUM 4096u

typedef enum {
    Z_DEG_001_INT = (Z_FIX_ANGLES_NUM / 360),
    Z_DEG_022_INT = (Z_FIX_ANGLES_NUM / 16),
    Z_DEG_045_INT = (Z_FIX_ANGLES_NUM / 8),
...
typedef enum {
    Z_DEG_001_FIX = (Z_FIX_ANGLES_NUM * Z_FIX_ONE / 360),
...
extern const ZFix z_fix__sin[Z_FIX_ANGLES_NUM];
extern const ZFix z_fix__csc[Z_FIX_ANGLES_NUM];
...
{
    return Angle & (Z_FIX_ANGLES_NUM - 1);
}

static inline ZFixu z_fix_angleWrapf(ZFixu Angle)
{
    return Angle & (Z_FIX_ANGLES_NUM * Z_FIX_ONE - 1);
}

static inline unsigned z_fix_angleFromDeg(unsigned Degrees)
{
    return Z_FIX_ANGLES_NUM * Degrees / 360;
}

static inline ZFixu z_fix_angleFromDegf(unsigned Degrees)
{
    return (ZFixu)((uint64_t)(Z_FIX_ONE * Z_FIX_ANGLES_NUM) * Degrees / 360);
}
