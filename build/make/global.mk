Z_PROJECT_ROOT := $(realpath $(dir $(lastword $(MAKEFILE_LIST)))/../..)

F_CONFIG_APP_AUTHOR := alxm
F_CONFIG_APP_NAME := Trap75

F_CONFIG_APP_VERSION_MAJOR := 0
F_CONFIG_APP_VERSION_MINOR := 2
F_CONFIG_APP_VERSION_MICRO := 0

F_CONFIG_COLOR_SCREEN_BORDER := 0x201040

F_CONFIG_DIR_MEDIA := assets
F_CONFIG_DIR_SRC := trap75

F_CONFIG_FILES_EMBED_OBJ_SPRITE := $(shell find $(Z_PROJECT_ROOT)/assets/gfx -name "*.png")
F_CONFIG_FILES_EMBED_OBJ_SPRITE := $(F_CONFIG_FILES_EMBED_OBJ_SPRITE:$(Z_PROJECT_ROOT)/%=%)
F_CONFIG_FILES_EMBED_OBJ_SAMPLE := $(shell find $(Z_PROJECT_ROOT)/assets/sfx -name "*.wav")
F_CONFIG_FILES_EMBED_OBJ_SAMPLE := $(F_CONFIG_FILES_EMBED_OBJ_SAMPLE:$(Z_PROJECT_ROOT)/%=%)

F_CONFIG_SCREEN_FORMAT := F_COLOR_FORMAT_RGB_565
F_CONFIG_SCREEN_RENDER := SOFTWARE
F_CONFIG_SCREEN_SIZE_WIDTH := 80
F_CONFIG_SCREEN_SIZE_HEIGHT := 64

F_CONFIG_SOUND_VOLUME_ADJUSTABLE := 0
F_CONFIG_SOUND_VOLUME_SCALE_MUSIC := 20
F_CONFIG_SOUND_VOLUME_SCALE_SAMPLE := 20

F_CONFIG_TRAIT_LOW_MEM := 1
